...
def build_movie_edit_field_keyboard(movie_id: int) -> InlineKeyboardMarkup:
    """Return the field picker shown after tapping "✏️ Tahrirlash" on a movie."""
    rows = [
        [
            InlineKeyboardButton(
                text="📝 Nomi",
                callback_data=MovieActionCallback(action="edit_title", movie_id=movie_id).pack(),
            ),
            InlineKeyboardButton(
                text="📅 Yili",
                callback_data=MovieActionCallback(action="edit_year", movie_id=movie_id).pack(),
            ),
        ],
        [
            InlineKeyboardButton(
                text="🎭 Janri",
                callback_data=MovieActionCallback(action="edit_genre", movie_id=movie_id).pack(),
            ),
            InlineKeyboardButton(
                text="📄 Tavsifi",
